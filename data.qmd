...
#### Disability 
```{r}
disability_r <- read_csv("data/reading/NAEP_Disability.csv")
disability_r <- disability_r[8:40, ]
colnames(disability_r) <- as.character(disability_r[1, ])
disability_r <- disability_r[-1, ]
```

```{r}
disability_r$Year <- as.Date(paste0(disability_r$Year, "-01-01"))

ggplot(disability_r, aes(x = Year, y = `Average scale score`, color = `Disability status of student, excluding those with 504 plan`)) +
  geom_point() +
  geom_line(aes(group = `Disability status of student, excluding those with 504 plan`)) +
...
#### Gender 
```{r}
gender_r <- read_csv("data/reading/NAEP_Gender.csv")
...
```{r}
gender_r$Year <- as.Date(paste0(gender_r$Year, "-01-01"))

ggplot(gender_r, aes(x = Year, y = `Average scale score`, color = `Gender`)) +
  geom_point() +
  geom_line(aes(group = `Gender`)) +
  scale_x_date(breaks = as.Date(paste0(seq(1978, 2024, 4), "-01-01")), 
               date_labels = "%Y") +
  theme_minimal() + 
  theme(legend.position = "bottom")
```
...
```{r}
lunch_program_r <- read_csv("data/reading/NAEP_LunchProgram.csv")
...
```{r}
lunch_program_r$Year <- as.Date(paste0(lunch_program_r$Year, "-01-01"))
...
#### Race Ethnicity 
```{r}
race_r <- read_csv("data/reading/NAEP_Race.csv")
race_r <- race_r[8:104, ]
...
```{r}
race_r$Year <- as.Date(paste0(race_r$Year, "-01-01"))
...
#### School Region
```{r}
region_r <- read_csv("data/reading/NAEP_Region.csv")
...
```{r}
region_r$Year <- as.Date(paste0(region_r$Year, "-01-01"))

ggplot(region_r, aes(x = Year, y = `Average scale score`, color = `Region of the country`)) +
  geom_point() +
  geom_line(aes(group = `Region of the country`)) +
  scale_x_date(breaks = as.Date(paste0(seq(2004, 2024, 4), "-01-01")), 
               date_labels = "%Y") +
...
#### School Location 
```{r}
location_r <- read_csv("data/reading/NAEP_Location.csv")
...
```{r}
location_r$Year <- as.Date(paste0(location_r$Year, "-01-01"))
...
#### Status as English Learner
```{r}
english_status_r <- read_csv("data/reading/NAEP_EnglishStatus.csv")
...
```{r}
english_status_r$Year <- as.Date(paste0(english_status_r$Year, "-01-01"))
